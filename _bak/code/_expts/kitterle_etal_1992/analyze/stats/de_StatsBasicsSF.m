function stats = de_StatsBasicsSF(mSets, mss, dump)
  ...
  for mi=1:length(mss)
      ...
      if (any(guru_instr(mSets.data.train.TLAB, 'freq')))
          fprintf('Sigma: %5.2f:\tFr1:%5.2e\tFr2:%5.2e\n', mss{mi}(1).sigma, mean(mean(stats.perf{mi}(:, 1, :))), mean(mean(stats.perf{mi}(:, end, :))));
          
      elseif (any(guru_instr(mSets.data.train.TLAB, 'sin')))
          fprintf('Sigma: %5.2f:\tsin:%5.2e\tsqu:%5.2e\n', mss{mi}(1).sigma, mean(mean(stats.perf{mi}(:, :, 1))), mean(mean(stats.perf{mi}(:, :, 2))));
