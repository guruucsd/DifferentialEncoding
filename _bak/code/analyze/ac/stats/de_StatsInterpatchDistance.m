...
% ti : # of training iterations
  ...
  for ss=1:length(models)
      ...
      for mm=1:length(ms)
          ...
          c = m.ac.Weights~=0;
          cc=reshape( full(c(inPix+1:inPix+m.nHidden, 1:inPix)), [m.nHidden m.nInput]);
...
%                dists(mm,hh,1:length(cx)) = 0;
      ...
      ipd.mean(ss) = mean(dists(dists(:)>0)); %inter-patch distance
      ipd.std(ss)  = std(dists(dists(:)>0));
      ipd.mean2(ss) = mean(d2(d2(:)>0));      %average distance from center
      ipd.std2(ss) = std(d2(d2(:)>0));
  end;
