...
if ~isempty(bw_title)
  handles.title = title(bw_title, 'fontsize',get(gca,'fontsize')+4);
else
  handles.title = [];
end
if ~isempty(bw_xlabel)
  handles.xlabel =xlabel(bw_xlabel, 'fontsize',get(gca,'fontsize'));
else
  handles.xlabel = [];
end
if ~isempty(bw_ylabel)
  handles.ylabel =ylabel(bw_ylabel, 'fontsize',get(gca,'fontsize'));
else
  handles.ylabel = [];
end

set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0]);
mfe_xticklabels(gca,1:numgroups,groupnames);
...
xwidth = width; %5 / (width * numbars /numgroups);
for i = 1:numbars
  x = (1:numgroups) - groupwidth/2 + (2*i-1) * groupwidth / (2*numbars);
  handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), errors(:,i), 'k', 'LineStyle', 'none', 'LineWidth', 2);
end
for i=1:numbars
  mfe_errorbar_tick(handles.errors(i), xwidth/3, 'units', 'top');
end;


if ~isempty(bw_legend)
  handles.legend = legend(handles.bars, bw_legend, 'location', 'NorthWest', 'fontsize',get(gca,'FontSize'));
  legend boxoff;
else
  handles.legend = [];
end
