  ...
  for i=1:length(out.data)
    switch(out.data{i})
      ...
      case 'mat'
        ...
        out.files{end+1} = de_GetOutFile(mSets, 'data', '.mat', mSets.runs);%fullfile(out.datapath, [out.stem '.mat']);
      ...
      case 'csv'
        out.files{end+1} = de_GetOutFile(mSets, 'summary', '.csv', mSets.runs);%fullfile(out.datapath, [out.stem '.csv']);
        %out.files{end}   = guru_smartfn( out.files{end} );
      ...
      case 'info'
        out.files{end+1} = de_GetOutFile(mSets, 'summary', '.summary.txt', mSets.runs);%fullfile(out.datapath, [out.stem '.mat']);
      ...
      % Save run results to .txt
      case 'cmd'
        out.files{end+1} = de_GetOutFile(mSets, 'data', 'summary', '-replicate.m', mSets.runs);%out.data{i});%fullfile(out.datapath, [out.stem '.mat']);
