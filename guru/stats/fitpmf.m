function [p,mn,var] = fitpmf(pmf, distn, bins, guess, showfig)
...
% var : variance of parameterized distribution
    ...
    dndist = @(d1,d2) (sum( (d1(1:end-1) - d2(1:end-1)).^2 ));
    fitfn = @(data,x,pmf,p) (dndist(data,pmf(x,p{:})));

    % Fitting functions
    [p] = fminsearch(@(p) fitfn(distn, bins, pmf, num2cell(p)), guess, optimset('MaxIter', 1000, 'Display', 'none'));
    ...
    if showfig
        bar(bins, distn);
        hold on;
        xvals = 0:0.01:max(bins); spacing_fact = mean(diff(bins))/0.01;
        pcell = num2cell(p);
        p1 = pmf(xvals, pcell{:})*spacing_fact;
