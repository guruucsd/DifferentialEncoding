...
% filterParams: maxfreq, minfreq, [minFreq maxFreq] (in cycles/image)
...
%
% Example:
% [outimg] = guru_filterImages(img, 'lowpass', 5, true); % plots img&outimg
  ...
  switch (filterType)
      % Recursive calls for lowpass & highpass
      ...
      case 'bandpass'
        ...
        for ii=1:num_images
          ...
          guru_assert(all(isreal(filt_image(:))), 'ifft images are expected to be real numbers');
        end;
  ...
  filt_images = squeeze(filt_images); % remove singleton first dimension, if it exists.
