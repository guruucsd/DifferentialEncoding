...
if eval(feval(mean{:})) ~= numel(hyp.mean)
  error('Number of mean function hyperparameters (%d) disagree with mean function (%d)', numel(hyp.mean), eval(feval(mean{:})))
end
if ~isfield(hyp,'cov'), hyp.cov = []; end
if eval(feval(cov{:})) ~= numel(hyp.cov)
  error('Number of cov function hyperparameters (%d) disagree with cov function (%d)', numel(hyp.cov), eval(feval(cov{:})))
end
