function [model,ws,s,fs] = autoencoder(model, ws, exts)
...
%  if (~isfield(model, 'MaxIterations')), model.MaxIterations = 50;          end;    % # iterations through the training set
  ...
  %% Do analyses
  [s,fs] = do_analysis(model, ws, exts);
...
function [s,fs] = do_analysis(model, ws, exts)
...
%    fprintf('Std:    %6.4e => %6.4e\n', std   (s.dist_avg_o),    std(s.dist_avg_e));
  ...
  fs = cell(3 * length(exts) + 1, 1);
  if (nargout == 1)
      for pfi=1:length(exts)
        fs{3 * (pfi - 1) + 1} = sprintf('z_recon.%s', exts{pfi});
        fs{3 * (pfi - 1) + 2} = sprintf('z_conn.%s', exts{pfi});
        fs{3 * (pfi - 1) + 3} = sprintf('z_hist.%s', exts{pfi});
      end;
      fs{end} = 'model.mat';
  else
      for pfi=1:length(exts)
        for fi=1:3
            fs{3 * (pfi - 1) + fi} = sprintf('%s.%s', tempname(), exts{pfi});
        end;
      end;
