function [mean_mean_nn_dist,mean_nn_dist,nn_dist] = estimate_spread(model,npts)
if ~exist('npts','var'), npts = 33; end;

mean_nn_dist = zeros(npts,1);
nn_dist = zeros(npts,sum(1:(model.nConns-1)));
for ii=1:npts
    if false % discrete case
        ...
        [mean_nn_dist(ii),~,nn_dist(ii,:)] = de_calc_nn_dist(all_samps(:,1), all_samps(:,2));
    end;
end;
