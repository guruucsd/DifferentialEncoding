function [cargs, opts] = uber_args(varargin)

  [cargs,opts] = common_args( ...
            'runs',    25, ...
            'errorType', 3, ...
            ...
            'ac.zscore', 0.05, ...
            'ac.TrainMode','resilient-homeostatic', ...
            'ac.Pow', 1, ... %gradient power (usually 1)
            'ac.EtaInit', 7E-2, ...
            'ac.Acc', 5E-5, ...
            'ac.Dec', 0.15, ... %5E-7 tanh#2, bias=1 resilient
            'ac.MaxIterations', 100, ...
            ... % don't train to error, just max out on noise
            'ac.AvgError', 0, ...2.01E-4, ...
            'ac.lambda', 0.0, ...
            'ac.noise_input', 0.02, ...
            'ac.avgact', 0.05, ...
            'ac.bn', 3.3E-1, 'ac.bc', 3.3E-2, ...
            'ac.rej.width', [14], 'ac.rej.type', {'sample_std-normd'}, ...
            ... % or train to a criterion level with weight decay
            'ac.AvgError', 2E-4, ...
            'ac.lambda', 0.01, ...
            'ac.dropout', 0.0, ...
            'ac.noise_input', 0.004, ...
            'ac.stdact', 0.01, ...
            'ac.bn', 3.3E-4, 'ac.bc', 3.3E-5, ...
...%            'ac.rej.width', [3], 'ac.rej.type', {'sample_std-normd'}, ...
            'ac.rej.width', [NaN], 'ac.rej.type', {'max'},...sample_std-normd'}, ...
            'p.errorType', 3,... % cross-entropy
            'p.XferFn', [6 3], ...  %sigmoid->sigmoid
            'p.TrainMode', 'resilient', ...
            'p.EtaInit', 5E-2, ...
            'p.Acc', 1E-7, ...
            'p.Dec', 0.25, ...
              ...
             varargin{:} ...
           );


% opts = {opts{:}, 'img2pol','location','LVF'};
