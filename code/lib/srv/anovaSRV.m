function [p,anovatab,stats] = anovaSRV(x,group,displayopt,extra)
...
% willdisplay fixed to O instead of next line (commented by EBI 28/06/12) because it seems to bug when the group sizes are not the same (which can be the case after discarding outlierss
