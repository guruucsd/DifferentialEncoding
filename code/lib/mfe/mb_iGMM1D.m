function mb_iGMM1D(data,numOfIterations,numOfBurnInIter)
...
if ~exist('numOfIterations','var'), numOfIterations = 100; end;
if ~exist('numOfBurnInIter','var'), numOfBurnInIter =  30; end;
if exist('data','var'),
    numOfDataPoints = numel(data); 
    data = data(:);
    [~,bins] = hist(data,floor(numOfDataPoints/10));
else, 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% produce data & initialize output
if ~exist('data','var'), 
    [data,bins] = produceData(numOfDataPoints,clusterFigHandle); 
else
    bar(bins, histc(data, bins)/numOfDataPoints/diff(bins(1:2)), 'EdgeColor','none');
end;
...
%invgampdf = @(x,alpha,beta) (beta.^alpha/gamma(alpha)*x.^(-alpha-1).*exp(-beta./x));
%c1=-3/2; c2=-1/2;
c1=-3/2; c2=-1/2;

alphaProbDensity   = @(x)x.^(c1).*exp(c2./x); % eq 14: inv-gamma(1/2,1/2)
%alphaProbDensity   = @(x)x.^(-3/2).*exp(-1/2./x); % eq 14: inv-gamma(1/2,1/2)
%alphaProbDensity   = @(x) invgampdf(x,1.5,0.5);
...
while iter < numOfIterations
    ...
    alphaProbDensity = @(x)exp( (numOfClasses+c1)*log(x) +c2/x + gammaln(x) - gammaln(x+numOfDataPoints) + gammaln(numOfDataPoints));
...
function [data,bins] = produceData(numOfDataPoints,clusterFigHandle)
...
data              = randn(1,numOfDataPoints)./(true_precs(missingData)).^.5 + true_mus(missingData);
        ...
        figure(clusterFigHandle);
        [counts,bins] = hist(data,numOfDataPoints/10);
