...
% Train with basic backprop, in batch mode
  ...
  for ip = 1:model.MaxIterations
    ...
    if (any(isnan(model.Weights(:)))), error('some weight are nan :-/'); end;
    if (isfield(model, 'wmax')), model.Weights = sign(model.Weights).*min(abs(model.Weights), model.wmax); end; % max weights
