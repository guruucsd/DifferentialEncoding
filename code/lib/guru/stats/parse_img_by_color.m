function [data,pix,xticks,yticks] = parse_img_by_color(img_file, data_colors, xtick_fn, ytick_fn, tol)
%function [data,pix,xticks,yticks] = parse_img_by_color(img_file, data_colors, xtick_fn, ytick_fn, tol)
...
%    if ~iscell(xticks_vals), xticks_vals = {xticks_vals}; end;
 %   if ~iscell(yticks_vals), xticks_vals = {yticks_vals}; end;
    ...
    [yticks{1},yticks{2}]  = get_pixels_by_color(img_file, 'y', 0, tol); %yticks = sort(yticks);  % 2 cols of them
    [xticks{1},xticks{2}]  = get_pixels_by_color(img_file, 'm', 0, tol); %xticks = sort(xticks);

    pix = cell(length(data_colors),2);
    data = cell(size(pix));
    for ci=1:length(data_colors)
        [pix{ci,1},pix{ci,2}] = get_pixels_by_color(img_file, data_colors{ci}, 0, tol);  % total fibers
