...
function [Con,mu] = de_connector2D(sI,sH,hpl,numCon,distn,rds,sig,dbg,tol)
  %
...
%                     d = 2*pi*rand + d/2
 ...
 %                     d = 2*pi*rand + (pi/2/theta)*randn(size(r));
          ...
          guru_assert(size(pix,1)==numCon, sprintf('Failed to sample %d connections after %d tries.', numCon, tryi));
    ...
    if (nNotCon==0)
        guru_assert(sum(Con(:)) == 2*sH*numCon);
        
    else
        x      = dbstack();
        nCalls = sum(strcmp(x(1).name, {x.name}));
        if (nCalls >= 100), error('Failed to connect network to ALL inputs/outputs after %d calls; quitting...', nCalls); end;
        
%        if (dbg), fprintf('[%d unconnected pix]', nNotCon); end;
        if (dbg), fprintf('.'); end;
       ...
       %    fprintf('%6.4f<=%6.4f', nNotCon/prod(sI),tol);
        end;
    end;
