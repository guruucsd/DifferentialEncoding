...
% h             : array of handles to plots
  ...
  %2d plot
    fig = guru_newFig('image-fft-inst', 'images', dset);
   ...
   fig(2) = guru_newFig('image-fft-instavg', 'images', dset);
   means = mean(ffts,1);
   de_PlotOutputImageFFTs_2D( ...
       means,...
       dset, ...
  ...
  %2d plot
    fig = guru_newFig('image-fft-avg', 'images', dset);
   ...
   % Plot average over images
   fig(2) = guru_newFig('image-fft-avgavg', 'images', dset);
   means = mean(ffts,1);
   de_PlotOutputImageFFTs_2D( ...
       means,...
       dset, ...
