function [stats_opt] = de_StatsOptimal(huacts,dset)
%
% Compares to the optimal
  ...
  comparison_ideal_global = cell(size(huacts));
  comparison_ideal_local  = cell(size(huacts));
  
  for i=1:length(huacts)
    comparison_ideal_global{i} = zeros([size(huacts{i},1) 1]);
    comparison_ideal_local{i} = zeros([size(huacts{i},1) 1]);
    
    for j=1:size(huacts{i},1)%#models
      huact = reshape(huacts{i}(j,:,:), [size(huacts{i},2), size(huacts{i},3)]);

      % Correlate hidden unit activations
      corrs = zeros(nStims);
      for w1=1:nStims
        for w2=1:nStims
          corrs( w1,w2 ) = corr( huact(:, w1), huact(:,w2) );
        end;
      end;
