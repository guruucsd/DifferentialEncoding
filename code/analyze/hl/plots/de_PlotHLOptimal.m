function [figs] = de_PlotHLOptimal(huacts,dset)
%
% huacts: [nModels] x [nHidden] x [nStim]
%
% Compares to the optimal

  %
  nModels = size(huacts,1);
  nHidden = size(huacts,2);
  nStims  = size(huacts,3);
  ...
  %  
  for i=1:nModels
    huact = reshape(huacts(i,:,:), [nHidden, nStims]);

    % Correlate hidden unit activations
    for w1=1:nStims
      for w2=1:nStims
        corrs(i,w1,w2 ) = corr( huact(:, w1), huact(:,w2) );
      end;
    end;
