...
%   hash    -
    ...
    case 'conn'
        ...
        origString = [ origString ...
                       sprintf('DS=%s,',   ws.dataset), ...
                       sprintf('IP=[%s],', sprintf(' %d',ws.iters_per)), ...
                       sprintf('ST=[%s],', sprintf(' %d',ws.steps)), ...
                       sprintf('NP=%d,',   ws.npruning_loops), ...
                       sprintf('PL=%s,',   ws.prune_loc), ...
                       sprintf('PS=%s,',   ws.prune_strategy), ...
                       sprintf('KW=%d,',   ws.keep_weights) ...
                     ];

      hash = origString;% sprintf('%d', round( sum(origString.*[1:5:5*length(origString)]) ));
      file = sprintf('%s-%d.conn', hash, model.ac.randState);

      fullpath = true; % this is a module of sorts..
    ...
    case 'p',
      origString = [...%sprintf('RS=%f', model.p.randState), ... %this is marked on the file
                     sprintf('NI=%d', model.p.noise_input), ...
