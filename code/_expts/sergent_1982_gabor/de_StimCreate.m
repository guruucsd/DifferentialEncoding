...
%  test.*     : same as train object, but
  ...
  nScales = guru_getopt(opt, 'nScales', 2);
  nOrient = guru_getopt(opt, 'nOrient', 4);
  nInput  = guru_getopt(opt, 'nInput', [31 13 nScales, nOrient]);
...
%        T(end+1,:)=double(~ismember(TT, [LpSpID LpSpNID])); % is there a distracter anywhere?
  ...
  function [X,STIM,ST] = stim2D(nInput)
    ...
    if (~any(nInput(1:2)-[31 13]))
        ...
        % Rescale image
        X = zeros([nInput(1:2) size(I,2)]);
        for i=1:size(I,2)
            % Scale one dimension
            tmp    = imresize(reshape(I(:,i), [31 13]), min(xscale,yscale));

            % Add padding
            npad = max(nInput(1:2) - size(tmp));
            npad1 = round(npad/2);
            npad2 = npad - npad1;
        ...
        X = reshape(X, [prod(nInput(1:2)) size(X,3)]);
    end;

    % Now duplicate layers as often as necessary, and apply gabor filters
    gaborfilters = gfCreateFilterBank(nInput(3), nInput(4), nInput(1:2), 1);
    X2 = zeros([size(X,1) nInput(3:end) size(X,2)]);

    for i=1:size(X,2)
        filteredimg = gfFilterImage(reshape(X(:,i), nInput(1:2)), gaborfilters, 1);
        X2(:,:,:,i) = reshape(vertcat(filteredimg{:}),[size(X,1) nInput(3:end)]);

            figure; jj=1;
        for j=1:nInput(3)
            for k=1:nInput(4)
                subplot(nInput(3), nInput(4), jj);
                imagesc( reshape(X2(:,j,k,i), nInput(1:2)) );
                jj = jj + 1;
            end;
        end;
    end;
    ...
    X = reshape( X2, [prod(nInput) size(X,2)]);
