...
stim_names = cellfun(@(mSets) sprintf('%%s', mSets.data.stimType), all_mSets, 'UniformOutput', false);
...
for si=1:2
    expt_idx = 2 * (si - 1) + [1:2];
    hemi_idx = [1 size(stats.mean_error, 2)];

    subplot(1, 2, si);
    errorbar([0 0; 1 1], stats.mean_error(expt_idx, hemi_idx), stats.std_error(expt_idx, hemi_idx), 'LineWidth', 3.0);
