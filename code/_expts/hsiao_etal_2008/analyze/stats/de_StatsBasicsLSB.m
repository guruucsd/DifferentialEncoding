function stats = de_StatsBasicsSF(mSets, mss, verbose)
  ...
  for mi=1:length(mss)
      ms    = mss{mi}; 
      p     = [ms.p];
      tmp   = de_calcPErr( vertcat(p.lastOutput), mSets.data.train.T, mSets.errorType);
  ...
  fprintf('\n');
  for mi=1:length(mss)
       fprintf('Sig=%5.2f\tYes: %5.2e +/- %5.2e\tNo: %5.2e +/- %5.2e\tTotal: %5.2e +/- %5.2e\n', mss{mi}(1).sigma, ...
             mean(stats.perf{mi}{1}(:)), std(stats.perf{mi}{1}(:)), ...
             mean(stats.perf{mi}{2}(:)), std(stats.perf{mi}{2}(:)), ...
             mean([stats.perf{mi}{1}(:);stats.perf{mi}{2}(:)]), ...
             std ([stats.perf{mi}{1}(:);stats.perf{mi}{2}(:)]));
  end;
