    ...
    for i=1:length(opt)
      ...
      switch (curopt)
          case {'small','medium','large'}
              ...
              nInput_new = size(imresize( reshape(X_new(:,1), nInput), scale ));
              X_new2  = zeros(prod(nInput_new), size(X_new,2));
              for i=1:size(X,2)
                x = reshape(imresize( reshape(X_new(:,i), nInput), scale ), [prod(nInput_new) 1]);
                X_new2(:,i) = (x - min(x))/(max(x)-min(x));
              end;
              guru_assert(~any(X_new2(:)<0), 'no values outside [0 1]');
              guru_assert(~any(X_new2(:)>1), 'no values outside [0 1]');
              X_new = X_new2;
...
%            keyboard; % need to verify
  ...
  function makeChimeric(indir, outdir, side)  
    fprintf('This is only for when you do not detect orig & chimeric pgm files.  They should be there, unless you''ve just unarchived this...\n');
