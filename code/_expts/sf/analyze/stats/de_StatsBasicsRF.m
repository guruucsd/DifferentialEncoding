...
for ti=1:2
  ...
  for mi=1:length(mss)
      ...
      tmp   = de_calcPErr( vertcat(o.(tt)), mSets.data.test.T, 2);
      %goodTrials = ~isnan(sum(mSets.data.(tt).T,1)); % only grab trials where

      S1_perf = tmp(:, guru_instr(mSets.data.(tt).TLAB,'S1')); %2 freqs
      S2_perf = tmp(:, guru_instr(mSets.data.(tt).TLAB,'S2')); %3 freqs
