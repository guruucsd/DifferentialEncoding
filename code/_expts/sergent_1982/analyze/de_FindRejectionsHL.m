function [rejMats] = de_FindRejectionsHL(models, rejSets, ...)
...
%                      mSets.data.LmSmID mSets.data.LmSmNID];
      ...
      switch rmodes{r}
        ...
        case 'ac-gauss'
          rejectTypes(:,r) = de_DoRejections_All('gauss', reshape(errAutoEnc,[numel(errAutoEnc),1]), ...
        ...
        case 'ls-gauss'
          rejectTypes(:,r) = de_DoRejections_All('gauss', reshape(LS,[numel(LS) 1]), ...
                                            de_SmartBins(LS), mSets.rej.width);
        case 'ls-exgauss'
          rejectTypes(:,r) = de_DoRejections_All('exgauss', reshape(LS,[numel(LS) 1]), ...
                                            de_SmartBins(LS), mSets.rej.width);
        case 'total-gauss'
        ...
        otherwise
          continue; %error('Unknown rejection mode: %s', rmodes{r});
      end; %switch
    end;
...
%      bins        = de_SmartBins(LS);
      ...
      for i=tidx
        ...
        % the mean & std
        data(data>bins(find(a,1,'last'))) = bins(find(a,1,'last'));
